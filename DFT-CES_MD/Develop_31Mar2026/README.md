...
This patch is intended for coupling DFT-CES with **MACE** (a machine-learning interatomic
potential). Because MACE operates in LAMMPS `metal` units (energy in eV, length in Å), the
patch has been adapted to work in `metal` units instead of the original `real` units.
...
This patch assumes the LAMMPS input script uses `units metal`, so that DFT-CES can be coupled
with MACE.

- In `src/fix_gridforce.cpp` (`post_force`), the original `real`-unit code multiplied energy
  and forces by `23.06092` (eV → kcal/mol). For `metal` units the energy is already in eV, so
  this factor is set to `1.0`.
- The Quantum ESPRESSO input conversions are **unit-independent** and left unchanged:
  `src/grid.cpp` converts the cube-file potential from Rydberg to eV (`× 13.60569253`) and the
  coordinates / grid spacing from Bohr to Å (`× 0.52917721`).

Unit flow:

```
Quantum ESPRESSO (Ry, Bohr)
   │  grid.cpp: Ry -> eV (x13.60569253), Bohr -> Å (x0.52917721)
   ▼
internal (eV, Å)
   │  fix_gridforce.cpp
   ▼
LAMMPS metal units (eV, Å)   # factor 1.0  (real units would need x23.06092)
```

> If you switch back to `units real`, restore the `23.06092` factor in `fix_gridforce.cpp`.
